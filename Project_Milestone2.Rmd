...
```
My project is aiming to do differential analysis of genes in lung cancer patients between Afican Americans and whites ethnicities. Here I am using RNA Seq 123 vignette to follow the instructions. We are using 3 major packages here, edgeR, limma and Glimma. 
- edgeR - limma workflow for analysing RNA-seq data that takes gene-level counts as its input, and moves through pre-processing and exploratory data analysis      before obtaining lists of differentially expressed (DE) genes. This analysis is enhanced through the use of interactive graphics from the Glimma package, that allows for a more detailed exploration of the data at both the sample and gene-level than is possible using static R plots and (Homo.sapiens) package used for ENSEMBL gene annotations. 
...
DATA PACKAGING:
  Reading in count data:
    Download the required files from GDC portal which are a part of TCGA. There are in total of 40 files. (African Americans,20 and Whites,20)
```{r}
...
```
All the 40 files are combined into a matrix of counts using edgeR library's readGE function. It forms a matrix of ENSEMBL gene IDs and 40 sample files.  
```{r}
...
ORGANIZING THE SAMPLE INFORMATION: 
  Dividing the sample files into 2 groups of 20 each. 
```{r}
...
```
Using regex expression we remove the version or anything after the '.' in ENSEMBL IDs to match it to the libraries. 
...
We only require symbol, chromosome number and ENSEMBL ID from the Homo sapiens library. 
...
head(genes)
```
The genes that are duplicated needs to be removed. 
...
```
Since we are dealing with large datasets, instead of heatmap we would require heatmap.plus function to solve the error. 
```{r}
library(gplots)
...
mycol <- colorpanel(1000,"white","black", "blue")
